...
import { INITIAL_STATE, IUser, UserActionContext, UserContext } from './context';
...
const UserProvider: FC<PropsWithChildren> = ({ children }) => {
    ...
    const loginUser = async (userLoginInfo: IUser) => {
    ...
    const createUser = async (userRegInfo: IUser) => {
        ...
        await fetch('https://localhost:44311/api/services/app/Customer/Create', {
            method: 'POST',
            cache: "no-cache",
            headers: {
                "Content-Type": "application/json",
    ...
    const getUserDetails = async (id: number) => {
        const token = localStorage.getItem("token");
        await fetch(`https://localhost:44311/api/services/app/Customer/GetByUserId?id=${id}`, {
            method: 'GET',
            cache: "no-cache",
            headers: {
                "Content-Type": "application/json",
...
export { UserProvider, useLoginActions, useLoginState, useUser };